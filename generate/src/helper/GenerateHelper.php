...
class GenerateHelper {
    ...
    const formTypes = [
        'text','select','switch',
        'switchGroup','display','checkbox',
        'editor'
    ];

    const logicTypes = [
        'facade','model','repository','controller'
    ];
...
//        return strtoupper($firstLetter).$otherLetters;
    ...
    protected function getGridContent($config){
        ...
        foreach ($arr as $column){
            $transInfo = "trans('backend.".$config['table_name'].".column.".$column."')";
            $content .= '$grid->'.$column."(".$transInfo.");"."\n\t\t\t";
        }
        return $content;
    }
    ...
    protected function getFormContent($config){
        ...
        foreach ($arr as $column){
            $transInfo = "'".$column."',trans('backend.".$config['table_name'].".column.".$column."')";
            $content .=  '$form->text('.$transInfo.');'."\n\t\t\t";;
        }
        return $content;
    }
    /**
